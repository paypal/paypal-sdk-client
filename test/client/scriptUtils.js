/* @flow */
import {
    getScriptUrl, getEnableFunding,
    getDisableFunding, getDisableCard,
    getBuyerCountry, getAmount,
    getUserIDToken, getCSPNonce,
...
describe(`script utils cases`, () => {
    ...
    it('getScriptUrl should return the src of the script element', () => {
        ...
        if (result !== 'https://test.paypal.com/sdk/js?client-id=abcxyz123') {
            throw new Error(`should found the script src "https://test.paypal.com/sdk/js?client-id=abcxyz123", but got: ${ result }`);
        }
    });
    ...
    it('getEnableFunding should return an empty array  when enable-funding is not configure', () => {
        ...
        if (result.length > 0) {
            throw new Error(`should return and empty array, but got: ${ result.toString() }`);
        }
    });
    ...
    it('getEnableFunding should return a valid array when enable-funding is configure', () => {
        ...
        if (result[0] !== 'paypal') {
            throw new Error(`should return a valid array ["paypal"], but got: ${ result.toString() }`);
        }
    });
    ...
    it('getBuyerCountry should return the buyer country', () => {
        ...
        if (result !== 'US') {
            throw new Error(`should return US as the buyer country, but got: ${ String(result) }`);
        }
    });
    ...
    it('getAmount should return and error when the amount format is not correct', () => {
        ...
        try {
            ...
            if (err.message !== 'Invalid amount: 10') {
                throw new Error(`should throw an Error with incorrect amount format message`);
            }
        }
    });
    ...
    it('getAmount should return the amount', () => {
        ...
        if (result !== '10.00') {
            throw new Error(`should return an amount equals to "10.00", but got: ${ String(result) }`);
        }
    });
    ...
    it('getUserIDToken return a token string', () => {
        ...
        if (result !== 'token') {
            throw new Error(`should return the "token" word, but got: ${ String(result) }`);
        }
    });
    ...
    it('getCSPNonce should return a data-csp-nonce string', () => {
        ...
        if (result !== 'csp-none') {
            throw new Error(`should return the "csp-none" word, but got: ${ String(result) }`);
        }
    });
    ...
    it('getEnableThreeDomainSecure should return "true"', () => {
        ...
        if (!result) {
            throw new Error(`should has enable the three domain secure, but got: ${ String(result) }`);
        }
    });
    ...
    it('isChildWindow should return "false" when is not a child zoid window', () => {
        ...
        if (result) {
            throw new Error(`shouldn't be a child zoid window, but got: ${ String(result) }`);
        }
    });
});
