...
describe('graphql cases', () => {
    ...
    it('callGraphQL should fail with status code 404 when the URL was not found', async () => {
        ...
        try {
            ...
            if (err.message !== '/graphql returned status 404') {
                throw new Error(`should throw an error message "/graphql returned status 404", but got: ${ err.message }`);
            }
        }
    });
    ...
    it('callGraphQL should throw an exception when the response body contains errors', async () => {
        ...
        try {
            ...
            if (JSON.stringify(sourceData.errors[0]) !== err.message) {
                throw new Error(`should throw an error message "${ sourceData.errors[0] }", but got: ${ err.message }`);
            }
        }
    });
    ...
    it('callGraphQL should return a valid body response', async () => {
        ...
        if (sourceData.data.received !== received) {
            throw new Error(`should return a valid data response, but got: ${ String(received) }`);
        }
    });

    it('getGraphQLFundingEligibility should throw an error when fundingEligibility is not in the response', async () => {
        const expectedErrorMessage  = 'GraphQL fundingEligibility returned no fundingEligibility object';

        try {
            await getGraphQLFundingEligibility('fields');
        } catch (err) {
            if (err.message !== expectedErrorMessage) {
                throw new Error(`should throw an error message "${ expectedErrorMessage }", but got: ${ err.message }`);
            }
        }
    });
    ...
    it('getGraphQLFundingEligibility should return the fundingEligibility', async () => {
        ...
        if (!result) {
            throw new Error(`should return finding eligibility as "true", but got: ${ result }`);
        }
    });
});
