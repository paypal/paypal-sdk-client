...
describe(`domains test`, () => {
    ...
    it('getPayPalLoggerDomain should return the logger domain when is a local environment', () => {
        ...
        if (result !== 'https://mock://msmaster.qa.paypal.com') {
            throw new Error(`should get the logger domain "https://mock://msmaster.qa.paypal.com", but got: ${ result }`);
        }
    });
    ...
    it('getPayPalLoggerDomain should thrown an Error when is a local environment and the stage host is undefined', () => {
        ...
        try {
            ...
            if (err.message !== 'No stage host found') {
                throw new Error(`should thrown exception with message "No stage host found", but got ${ err.message }`);
            }
        }
    ...
    it('getAuthAPIUrl should return a valid authentication string URL', () => {
        ...
        if (`${ url.protocol }//${ url.hostname }` !== 'http://localhost' ||
            url.pathname !== '/v1/oauth2/token') {
            throw new Error(`should get the logger domain "${ window.location.protocol }//${ window.location.host }/v1/oauth2/token", but got: ${ url.toString() }`);
        }
    });
    ...
    it('getOrderAPIUrl should return a valid order string URL', () => {
        ...
        url.pathname !== '/v2/checkout/orders') {
            throw new Error(`should get the logger domain "${ window.location.protocol }//${ window.location.host }/v2/checkout/orders", but got: ${ url.toString() }`);
        }
    });
});
