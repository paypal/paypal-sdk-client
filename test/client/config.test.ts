...
describe(`config cases`, () => {
  it("should successfully get the paypal logger domain", () => {
    ...
    if (getPayPalLoggerDomain() !== expectedPayPalDomain) {
      throw new Error(
        `Expected paypal logger domain to be ${expectedPayPalDomain}, got ${getPayPalLoggerDomain()}`
      );
    }
  });
  it("should successfully build a paypal url", () => {
    ...
    if (result !== expectedPayPalUrl) {
      throw new Error(
        `Expected paypal url to be ${expectedPayPalUrl}, got ${result}`
      );
    }
  });
  it("should successfully build a paypal api url", () => {
    ...
    if (result !== expectedPayPalUrl) {
      throw new Error(
        `Expected paypal api url to be ${expectedPayPalUrl}, got ${result}`
      );
    }
  });
  it("should successfully build a paypal logger url", () => {
    ...
    if (result !== expectedPayPalUrl) {
      throw new Error(
        `Expected paypal logger url to be ${expectedPayPalUrl}, got ${result}`
      );
    }
  });
});
